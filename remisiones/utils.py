...
def _fmt_boolGestante(valSI, valNO) -> str:
	"""Return SI or NO for gestante"""
	if not valSI or not valNO:
		return "NO"
	elif valSI.strip.upper() == "X":
		return "SI"
	else:
		return "NO"
...
def _row_to_csv_record(row: tuple) -> list[str]:
	...
	gestSi, gestNo = ("SI", "") if _fmt_bool (c[6]) else ("", "SI")
	sexM, sexF     = ("SI", "") if _fmt_bool (c[8]) else ("", "SI")
...
def excelToDataframe ( excel_path: str | Path, header_rows: int = 10) -> "pd.DataFrame":
	"""
	...
	try:
		...
		print (f"+++ Saving to csv {excel_path=}")
		...
		print (f"+++ Error llevando excel to dataframe {ex=}")	
...
def excel_to_csv(excel_path):
	...
	_validate_structure(ws)

	outfile = os.path.basename (str(excel_path).split (".")[0] + ".csv")
	print (f"+++ {outfile=}")
